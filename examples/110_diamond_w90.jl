...
#
# Diamond WOHP from Wannier90 _hr.dat file
# No external dependencies beyond CrystalBonds.jl
#
# Usage: julia --project=.. examples/110_diamond_w90.jl

using CrystalBonds
using LinearAlgebra

# --- Read Wannier90 _hr.dat ---
hr_file = joinpath(@__DIR__, "..", "test", "fixtures", "diamond_hr.dat")
...
# --- Extract C-C bond (orbitals 1:4 = C1, 5:8 = C2) ---
cc_wohp = extract_bond(wohp, 1:4, 5:8)

# --- Orbital decomposition ---
wohp_ss = wohp.matrix[1, 5, :]
wohp_sp =
    dropdims(sum(wohp.matrix[1:1, 6:8, :]; dims = (1, 2)); dims = (1, 2)) .+
    dropdims(sum(wohp.matrix[2:4, 5:5, :]; dims = (1, 2)); dims = (1, 2))
...
# --- Plot (requires CairoMakie, optional) ---
try
    using CairoMakie
    ...
    outfile = joinpath(@__DIR__, "110_diamond_w90.png")
